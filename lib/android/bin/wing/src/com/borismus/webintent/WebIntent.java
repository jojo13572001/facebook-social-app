...
public class WebIntent extends CordovaPlugin  {
	...
	private static String gMyMessage;
	...
	public boolean execute(String action, JSONArray args, CallbackContext callbackContext) {
	    try {
	        ...
	        /***Bean Temporally add the function****/
	        else if (action.equals("recvBroadcast")) 
	        {
	            if (args.length() != 1) {
	                //return new PluginResult(PluginResult.Status.INVALID_ACTION);
	                callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.INVALID_ACTION));
	            ...
	            gECB = (String) obj.getString("ecb");
	            gMyMessage = (String) obj.getString("action");
	            Log.d("MatchingReceiver", "Start registerReceiver");
	            ((DroidGap)this.cordova.getActivity()).registerReceiver(mBroadcast, new IntentFilter(gMyMessage));
	...
	/****Bean temporally add the member***/
	private BroadcastReceiver mBroadcast =  new BroadcastReceiver() {
        private static final String TAG = "MatchingReceiver";
        @Override
        public void onReceive(Context context, Intent intent) {
             // TODO Auto-generated method stub
            if(gMyMessage.equals(intent.getAction())){
            	 try {
	           	      String action = intent.getAction();
	           	      String channel = intent.getExtras().getString("com.parse.Channel");
	           	      JSONObject json = new JSONObject(intent.getExtras().getString("com.parse.Data"));
	           	      Log.d(TAG, "new got action " + action + " on channel " + channel + " with:");
		           	  String _d = "javascript:" + gECB + "('" + channel + "')";
		       		  Log.d(TAG, "prepare sendJavascript: " + _d);
	
		       		  if (gECB != null && gWebView != null) {
		       			  Log.d(TAG, "start sendJavascript: " + _d);
